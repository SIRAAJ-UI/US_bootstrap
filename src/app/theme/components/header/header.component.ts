 ...
 ************************************************/
   ...
   */
  getDynamicNavigationMenu(menuItem: any) {
    if (menuItem) {
      ...
      let mapIntoNumberArray = splitResult.map(function (x: any) {
        return parseInt(x);
      })
      ...
      this.menuItems = menuObject.filter(function (y: any) {
  ...
  private getAuthInfo() {
    ...
    this.loginUserService.getUserInfo().subscribe((data: any) => {
      if (data) {
        ...
        const spinner_loader: any = document.getElementById("spinner_loader");

        spinner_loader.style.display = "block";
        spinner_loader.style.opacity = "1";
        spinner_loader.style.visibility = "vissble";
