...
export class ElockboxExceptionListComponent implements OnInit, AfterViewInit {
  ...
  public exceptionListData : any= [];
  public statusList: any = [{id: 3, type: "All"}];
  public reasonList : any= [{id: 0, type: "All"}];
  public actionTakenList: any=[];
  ...
  initValues() {
    ...
    const previousDay: any = myProfileInfo['previousDay'];
  ...
  public getExceptionListData(postParams: any) {
    ...
    this.elockboxService.getExceptionListData(postParams).subscribe(response => {
      if(response['item1'].length){
        ...
        this.exceptionListData.map((item:any) =>{
  ...
  public onDateSelected(date: any, type: any) {
    switch (type) {
      case 'from_date':
...
*/
  ...
  public toggleDetails(index: any,type: any){
    this.expanded = (this.expanded !== index) ? index : "";
  }

  public updateElockbox(){
    let updatedExceptionList: any=[];
    let isValidUpdate: any=false;
    if(this.exceptionListData && this.exceptionListData.length){
      try {
        this.exceptionListData.map((response: any)=>{
  ...
  private getTableColumnList() {
    const tableHeaderArrayList: any = [{
